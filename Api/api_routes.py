...
def verify_file(filename):
    ...
    try:
        # 🔍 DEBUG DETALLADO
        current_app.logger.info(f"🔍 Verificando archivo: '{filename}'")
        current_app.logger.info(f"   - Tipo: {type(filename)}")
        current_app.logger.info(f"   - Longitud: {len(filename)}")
        current_app.logger.info(f"   - Repr: {repr(filename)}")
        
        # Buscar exacto
        file = GameFile.query.filter_by(filename=filename).first()
        current_app.logger.info(f"   - Query exacto: {file}")
        
        # Si no se encuentra, buscar case-insensitive
        if not file:
            file = GameFile.query.filter(GameFile.filename.ilike(filename)).first()
            current_app.logger.info(f"   - Query case-insensitive: {file}")
        
        # Si aún no se encuentra, mostrar todos los disponibles
        if not file:
            all_files = [f.filename for f in GameFile.query.limit(10).all()]
            current_app.logger.info(f"   - Archivos disponibles: {all_files}")
            
            return jsonify({
                "success": False,
                "error": "Archivo no encontrado",
                "filename": filename,
                "available_files": all_files[:5]  # Solo los primeros 5
            }), 404
        
        # 🎯 ARCHIVO ENCONTRADO
        current_app.logger.info(f"✅ Archivo encontrado: {file.filename}")
        
        # Verificar si el archivo físico existe
        file_path = os.path.join(current_app.config['UPLOAD_FOLDER'], 'files', file.filename)
        file_exists = os.path.exists(file_path)
        
        current_app.logger.info(f"   - Archivo físico existe: {file_exists}")
        current_app.logger.info(f"   - Ruta: {file_path}")
        
        response_data = {
            "success": True,  # ← 🎯 CAMBIO CRÍTICO
        ...
        # Si existe, verificar MD5
        if file_exists and file.md5_hash:
            try:
                from utils import calculate_file_hash
                calculated_md5 = calculate_file_hash(file_path, 'md5')
                
                response_data.update({
                    "calculated_md5": calculated_md5,
                    "md5_match": calculated_md5 == file.md5_hash,
                    "integrity_status": "valid" if calculated_md5 == file.md5_hash else "corrupted"
                })
        ...
        current_app.logger.info(f"📤 Respuesta: {response_data}")
