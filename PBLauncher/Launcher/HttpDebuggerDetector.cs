...
public static class HttpDebuggerDetector
{
    ...
    public static class Whitelist
    {
        ...
        private static readonly string WhitelistFilePath = Path.Combine(Application.StartupPath, "process_whitelist.json");
        ...
        public static void LoadWhitelist()
        {
            try
            {
                ...
                // Cargar lista personalizada si existe
                if (File.Exists(WhitelistFilePath))
                {
                    var json = File.ReadAllText(WhitelistFilePath);
                    var customWhitelist = JsonSerializer.Deserialize<string[]>(json);

                    if (customWhitelist != null)
                    {
                        foreach (var process in customWhitelist)
                        {
                            _whitelistedProcesses.Add(process);
                        }
                    }
